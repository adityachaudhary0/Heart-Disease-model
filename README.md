...
1.  **Support Vector Machine (SVM)**: A supervised learning model used for classification. In this project, an RBF kernel SVM is utilized.
2.  **Logistic Regression**: A linear model for binary classification, often used as a baseline for classification tasks.
...
2.  **Install Dependencies**: Ensure all necessary libraries (pandas, scikit-learn, seaborn, matplotlib, kagglehub) are installed. KaggleHub might require authentication or setting up API keys, if not already configured.
